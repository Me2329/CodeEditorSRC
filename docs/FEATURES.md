...
| 1004 | About a third of the memory for about a tenth of the step time | at that shape: float32, and a model small enough that the head dominated |
| 1005 | On a card at `xxl` it is about 0.5GB of 13GB, which is four percent | bf16 halves the logits, and 9.5GB of weights dwarfs them |
| 1006 | The README says the small number, not only the flattering one | |
| 1007 | The saving scales with batch x context x vocabulary, the cost with parameters | so it is worth most on a small model with a big vocabulary |
| 1008 | Exactly equal to `F.cross_entropy` in float64, value and gradient | |
| 1009 | Bit-identical gradient in float32 too | |
| 1010 | The float32 value differs by about 5e-7 on a loss of order 6 | reassociation from summing in a different order, and nothing more |
| 1011 | Not claimed to be bit-exact in float32 | the tests pin what is true, not what would sound better |
| 1012 | Twelve optimiser steps run both ways stay within 5e-7 the whole way | so the difference does not compound |
| 1013 | That comparison is a test, not a note | |
| 1014 | The answer does not depend on the chunk size | checked at 1, 3, 7, 20, 60 and 1000 |
| 1015 | Padding is excluded once, at the end, by the count that mattered | averaging per piece would weight a piece of two tokens like one of five hundred |
| 1016 | Padding falling on a chunk boundary is still excluded | |
| 1017 | A batch that is entirely padding returns zero, not NaN | `F.cross_entropy` returns NaN, and one NaN poisons every weight |
| 1018 | And still has a gradient path, so the step does not fall over | |
| 1019 | No logits are returned, because none were kept | not holding them is the point |
| 1020 | Unless the caller asks for all of them, which overrides chunking | a caller that scores the output itself needs them |
| 1021 | Works with no gradient at all, for evaluation | |
| 1022 | Recomputation can be switched off, and changes no answer | |
| 1023 | A chunk size of zero is refused rather than looping forever | |
...
## A fine-tuning set worth training on

| # | Feature | Notes |
| --- | --- | --- |
| 1075 | 5134 instruction examples, up from 12 | the file used to say "real fine-tuning wants thousands of these" |
| 1076 | Generated by a script that is committed, from sources that are checked | edit `build.py`, not the output |
| 1077 | The header says the count is not 5134 independent facts | a few hundred verified pieces, in the shapes a model has to recognise |
| 1078 | Several phrasings of one request map to one answer, on purpose | that is how a model learns they are one intent |
| 1079 | Hand-written seeds live in their own file and survive regeneration | the generator wrote over them once |
| 1080 | 708 examples mined from this repository's own Python | real functions, with the docstring somebody wrote as the instruction |
| 1081 | A method lifted from its class is dedented before it is used | |
| 1082 | And dropped entirely if it still does not parse | 88 blocks were invalid Python before this check existed |
| 1083 | Every Python answer is compiled before it is written | a model cannot tell plausible from correct, and neither will its output |
| 1084 | 150 from the frontend's TypeScript, by its JSDoc | |
| 1085 | 204 about the editor itself, from the runtime registry | the same file three layers already read, so the answers cannot drift |
| 1086 | A hand-written catalogue of 24 tasks in up to 12 languages | 173 implementations, each read before it was committed |
| 1087 | Translation pairs between them, which is the pairing a code model wants | |
| 1088 | Capped so translation stays just under half the set | it is the cheapest direction to generate and the easiest to overdo |
| 1089 | Explain-this and name-the-language, the opposite direction | |
| 1090 | Document-this: the same function with its docstring removed | the answer is one a person wrote |
| 1091 | Duplicates removed, and the shuffle is seeded | the file is reproducible |
| 1092 | Eleven tests, including that every Python answer compiles | |

## What the assistant is told it is

| # | Feature | Notes |
| --- | --- | --- |
| 1093 | The chat assistant's prompt names the failure modes, not just the format | |
| 1094 | Answer length matches question length | a summary of what you just said wastes the reader's time |
| 1095 | Follow the conventions in the file being changed, not your own | a patch that reads like another author wrote it gets rewritten |
| 1096 | Name the file you would need to see, rather than guessing at it | |
| 1097 | Say which part of an answer is the assumption | rather than presenting all of it at one confidence |
| 1098 | Inventing a plausible function name is named as the worst failure | it costs a run, a read of the error, and trust in the next answer |
| 1099 | Describe the bug's runtime behaviour, not just the fix | a description of the fix is not a description of the bug |
| 1100 | It is told it cannot run anything, and that the Agent panel can | |
| 1101 | The agent is told to report what happened, not what was supposed to | if the tests fail, say so and paste the failure |
